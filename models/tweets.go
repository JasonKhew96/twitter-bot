...
func (o *Tweet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(tweetPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert tweets, could not build conflict column list")
			}

			conflict = make([]string, len(tweetPrimaryKeyColumns))
			copy(conflict, tweetPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"tweets\"", updateOnConflict, ret, update, conflict, insert, opts...)
